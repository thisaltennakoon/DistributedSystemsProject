                    ...
                    if (self.chat_system.get_chat_room(data['roomid'])) or (self.user_owns_chat_room(thread_owner)) or (
                        ...
                        if leader_response['approved'] == 'false':
                            ...
                            self.sendall_json(connection,
                                              {"type": "createroom", "roomid": leader_response['roomid'],
                                               "approved": "true"})
                            thread_owner.join_room(self.chat_system.get_chat_room(leader_response['roomid']))
    ...
    def threaded_server(self, connection):
        with connection:
            ...
            if data['type'] == 'sayhello':
                ...
                print("Server: " + data['senderid'] + " said Server: " + data['leaderid'] + " is the new leader")
...
class Bully:
    def __init__(self, serverid, chatSystem):
        ...
        self.state = "coodinated" #coordinator, coodinated, offline, election, takenover, intermediate
    ...
    def run_election(self):
        ...
        for i in (self.serverList):
            ...
            if int(j) > self.nodeid:
                ...
                print("Server " + str(self.serverid) + " sends a election message to server " + str(i) + "\n")
        sleep(1)
        if(self.state != "takenover"):        
            self.send_elected_msg()
            self.setState("coordinator")
            print("Server " + str(self.serverid) +" is elected as the new leader")
        else:
    ...
    def send_election_msg(self, id):
        self.send_json_message(self.serverList[id].server_address,self.serverList[id].coordination_port,{"type": "start_election", "serverid": self.serverid})

    def send_elected_msg(self): 
        for i in self.serverList:
            self.send_json_message(self.serverList[i].server_address,self.serverList[i].coordination_port,{"type": "newleader", "leaderid": self.serverid, "senderid":self.serverid})
    ...
    def election_msg_received(self, id):
        ...
        self.send_json_message(self.serverList[id].server_address,self.serverList[id].coordination_port,{"type": "tookover", "serverid": self.serverid})
